...
// @ts-expect-error - `className` is not in DialogPortalProps
...
type baseOverlay = typeof AlertDialogPrimitive.Overlay
interface overlay extends baseOverlay {
	className?: string
}

const AlertDialogOverlay = React.forwardRef<React.ElementRef<overlay>, React.ComponentPropsWithoutRef<overlay>>(
...
>(({ className, ...props }, ref) => (
	<AlertDialogPortal>
		...
		<AlertDialogPrimitive.Content
			ref={ref}
			className={cn(
				"fixed z-50 grid w-full max-w-lg scale-100 gap-4 border bg-background-dark text-dark p-6 opacity-100 shadow-lg sm:rounded-lg md:w-full",
				className
			)}
			{...props}
		/>

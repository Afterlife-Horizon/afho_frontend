	...
	return (
		<div className="mx-auto grid grid-rows-[3rem_3rem_3rem_1fr] gap-[0.2rem] rounded-b-lg">
			...
			<ScrollDiv className="flex h-[calc(100dvh-2rem-21rem-8rem)] w-full flex-wrap pl-5 pr-3">
				{Object.keys(effects).map((effect: string) => {
					if (!isKeyOf(effect, effects)) return <></>
					const key = effect as keyof Effects
					if (typeof effects[effect] === "number")
						return <NumberSelect effects={effects} effect={key} handleEffectChange={handleEffectChange} />
					if (typeof effects[effect] === "boolean")
						return <BooleanSelect effects={effects} effect={key} handleEffectChange={handleEffectChange} />
					return <></>
				})}
			</ScrollDiv>
		</div>
